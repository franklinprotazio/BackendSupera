...
public interface TransferenciaRepository extends JpaRepository<Transferencia, Long> {
    ...
    List<Transferencia> getTransferencias(@Param("inicio") Date inicio, @Param("fim") Date fim, @Param("nome") String nome);
    ...
    @Query("SELECT t FROM Transferencia t WHERE (:inicio IS NULL OR :inicio = '' OR t.dataTransferencia >= TO_DATE(:inicio, 'yyyy-MM-dd')) " +
            ...
            "AND (:nome IS NULL OR :nome = '' OR t.nomeOperadorTransacao = :nome)")
    List<Transferencia> getTransferencias(@Param("inicio") String inicio, @Param("fim") String fim, @Param("nome") String nome);
